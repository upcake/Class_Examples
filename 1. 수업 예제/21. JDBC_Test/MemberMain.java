...
public class MemberMain {
	public static void menuPrint() {
		...
		System.out.println("회원 정보 출력 : S");	// searchAllInput()		→	memberSearchAll()
		...
		System.out.println("회원 전화 검색 : T");	// searchTelInput()		→	memberSearchTel()
...
//				System.out.println("신규 회원 등록 화면");
				...
				input.searchAllInput();		// 회원 정보 출력하는 서브 화면
			} else if (menu.equalsIgnoreCase("n")) {	
				input.searchNameInput();	// 회원 이름 검색하는 서브 화면
			} else if (menu.equalsIgnoreCase("a")) {	
				input.searchAddrInput();	// 회원 주소 검색하는 서브 화면
			} else if (menu.equalsIgnoreCase("t")) {	
				input.searchTelInput();		// 회원 전화 검색하는 서브 화면
